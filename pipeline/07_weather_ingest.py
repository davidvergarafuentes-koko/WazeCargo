...
def get_conn():
    return psycopg2.connect(
        ...
        # configurable so the same script runs against RDS (require) and a
        # local/containerised Postgres (disable); default preserves AWS behaviour
...
def run_aggregate(conn):
    ...
    # NOTE: the original version of this query used correlated subqueries
    # referencing s.ts while grouping by EXTRACT(YEAR/MONTH FROM ts). Postgres
    # rejects that ("subquery uses ungrouped column s.ts from outer query"), so
    # the step could never run. Rewritten with CTEs that materialise year/month
    # as real columns; semantics are unchanged and the O(n^2) correlated scans
    # over port_operability_score are gone.
    sql = f"""
    WITH base AS (
        SELECT *,
               EXTRACT(YEAR  FROM ts)::INTEGER AS yr,
               EXTRACT(MONTH FROM ts)::INTEGER AS mo
        FROM {PG_SCHEMA}.port_operability_score
        WHERE is_forecast = FALSE
    ),
    events AS (
        SELECT port, yr, mo, COUNT(*) AS n_closure_events
        FROM (
            SELECT port, yr, mo, risk_level,
                   LAG(risk_level) OVER (PARTITION BY port ORDER BY ts) AS prev_rl
            FROM base
        ) e
        WHERE risk_level = 4 AND (prev_rl IS NULL OR prev_rl < 4)
        GROUP BY port, yr, mo
    ),
    drivers AS (
        SELECT DISTINCT ON (port, yr, mo) port, yr, mo, closure_driver
        FROM (
            SELECT port, yr, mo, closure_driver, COUNT(*) AS c
            FROM base
            WHERE risk_level >= 3
            GROUP BY port, yr, mo, closure_driver
        ) d
        ORDER BY port, yr, mo, c DESC
    )
    ...
    SELECT
        ...
        b.yr,
        b.mo,
        ROUND(AVG(b.wave_height_m)::NUMERIC,    3),
        ROUND(MAX(b.wave_height_m)::NUMERIC,    3),
        ROUND(AVG(b.swell_height_m)::NUMERIC,   3),
        ROUND(MAX(b.swell_height_m)::NUMERIC,   3),
        ROUND(AVG(b.wave_period_s)::NUMERIC,    2),
        ROUND(AVG(b.wind_speed_kmh)::NUMERIC,   2),
        ROUND(MAX(b.wind_gust_kmh)::NUMERIC,    2),
        ROUND(AVG(b.operability_index)::NUMERIC,4),
        ROUND(MIN(b.operability_index)::NUMERIC,4),
        ROUND(100.0 * SUM(CASE WHEN b.risk_level = 0 THEN 1 ELSE 0 END) / COUNT(*), 2),
        ROUND(100.0 * SUM(CASE WHEN b.risk_level = 1 THEN 1 ELSE 0 END) / COUNT(*), 2),
        ROUND(100.0 * SUM(CASE WHEN b.risk_level = 2 THEN 1 ELSE 0 END) / COUNT(*), 2),
        ROUND(100.0 * SUM(CASE WHEN b.risk_level = 3 THEN 1 ELSE 0 END) / COUNT(*), 2),
        ROUND(100.0 * SUM(CASE WHEN b.risk_level = 4 THEN 1 ELSE 0 END) / COUNT(*), 2),
        COALESCE(ev.n_closure_events, 0),
        NULL,   -- max_consecutive_closure_h (computed separately if needed)
        dr.closure_driver,
        ROUND(AVG(CASE WHEN b.risk_level >= 3 THEN b.wave_direction_deg END)::NUMERIC, 1),
        ROUND(100.0 * SUM(CASE WHEN b.swell_dominant THEN 1 ELSE 0 END) / NULLIF(COUNT(*),0), 2),
        ROUND(100.0 * COUNT(*) / 730.0, 2),
        NOW()
    FROM base b
    LEFT JOIN events  ev ON ev.port = b.port AND ev.yr = b.yr AND ev.mo = b.mo
    LEFT JOIN drivers dr ON dr.port = b.port AND dr.yr = b.yr AND dr.mo = b.mo
    GROUP BY b.port, b.zone, b.yr, b.mo, ev.n_closure_events, dr.closure_driver
